...
if (
validation.status === "requires_user"
) {
...
return {
valid: false,
code: "FACIAL_POSITIONS_INCOMPLETE",
message:
"O cliente precisa ter exatamente 10 posições faciais armazenadas."
};
}
...
return {
valid: false,
code: "FACIAL_POSITIONS_INVALID",
message:
"As posições faciais devem corresponder exatamente às posições 1 a 10."
};
}
...
return {
valid: false,
code: "FACIAL_POSITION_REFERENCE_DUPLICATED",
message:
...
return {
valid: false,
code: "FACIAL_POSITIONS_NOT_VERIFIED",
message:
...
if (!cleanString(client.passportNumber)) {
return {
valid: false,
...
if (
  clients.length !== objectIds.length
) {
  throw new Error(
...
if (
  ...
  return {
    ...
    code:
      "IDENTITY_NOT_READY",
    message:
      "Existem candidatos sem as 10 posições faciais prontas.",
    passportErrors,
    facialErrors,
    preferenceErrors:
      preferences.errors
...
* 
* IMPORTANTE:
* preparação não significa login no VFS,
* não significa slot encontrado e não
* significa marcação concluída.
  */
  async prepare(
  applicationId,
  accountId
...
await transition(
  ...
  STATES.IDENTITY_PREPARATION,
  {
    ...
    reason:
      "Preparing stored facial profile"
  }
);
...
await transition(
  ...
  STATES.IDENTITY_READY,
  {
    ...
    reason:
      "All ten stored facial positions are ready"
  }
);
...
await transition(
  ...
  STATES.READY_FOR_AUTOMATION,
  {
    ...
    reason:
      "Passport, identity preparation and preferences are ready"
  }
);
...
* Inicia o radar.
* 
* Bot 2 deve chamar este método em vez
* de decidir sozinho se pode monitorar.
  */
  async startRadar(
  applicationId,
  accountId
...
* 
* O método não assume que o slot
...
* Inicia a fase de booking depois
* de um slot encontrado.
  */
  async beginBooking(
  applicationId,
  accountId
...
* 
* Não realiza upload diretamente.
...
* Regista falha de OTP sem transformar
* automaticamente toda falha em erro
* permanente.
  */
  async markOtpFailed(
  applicationId,
...
* 
* PAYMENT_PENDING não é tratado como erro.
  */
...
* 
* A aplicação pode voltar ao radar.
  */
  async markSlotLost(
  applicationId,
  accountId,
